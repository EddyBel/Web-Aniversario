      ...
      >
        {body.map((item) => (
          <Tab key={`${item.title}-${item.body}`} title={item.title}>
            <Card className="p-3 sm:p-5 flex justify-between gap-4 flex-wrap lg:flex-nowrap items-center flex-row bg-gradient-to-tl to-gray-950 from-black">
              {item.img ? <Image isBlurred src={item.img} className="w-full" /> : ''}
              <CardBody className="w-full text-sm text-justify sm:text-start sm:text-sm text-gray-400">
                {item.body}
              </CardBody>
            </Card>
          </Tab>
